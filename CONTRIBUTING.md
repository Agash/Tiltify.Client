# Contributing to Tiltify.Client
...
```bash
dotnet restore Tiltify.Client.slnx
dotnet build Tiltify.Client.slnx
dotnet test Tiltify.Client.slnx
```
...
```bash
dotnet test Tiltify.Client.slnx --filter "TestCategory!=Integration"
```
